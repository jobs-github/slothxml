...
namespace slothxml
{
    ...
    template<typename T>
    bool encode(const sample_template_t<T>& obj_val, xml_document_t& doc, xml_element_t& xml_val)
    {
        return encode_field(obj_val.val_, "val_", doc, xml_val);
    }

    template<typename T>
    bool decode(const xml_element_t& xml_val, sample_template_t<T>& obj_val)
    {
        return decode_field(xml_val, "val_", obj_val.val_, obj_val.has_val_);
    }
    ...
    {
        std::string json_val;
        if (!slothxml::encode(init_val, "object", json_val))
        {
            return false;
        }
        T out;
        if (!slothxml::decode(json_val, "object", out))
        {
            return false;
        }
        return init_val == out;
    ...
    {
        if (!slothxml::dump(init_val, "object", path))
        {
            return false;
        }
        T out;
        if (!slothxml::load(path, "object", out))
        {
            return false;
        }
        return init_val == out;
    ...
    void test()
    {
        ...
        slothxml::sample_struct_t sobj;
        set_template<slothxml::sample_struct_t>(sobj);
        items.push_back(test_item_t("test_base<sample_test::sample_struct_t>", test_base<slothxml::sample_struct_t>(sobj)));
        ...
        items.push_back(test_item_t("test_file<sample_template_t<sample_struct_t>>", test_file< sample_template_t<sample_struct_t> >(tmpobj, "test.xml")));
...
int main(int argc, char * argv[])
{
    if (2 == argc)
    {
        ...
        int64_t us = slothxml::perf_test(count);
